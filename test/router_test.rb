...
class RouterTest < Minitest::Test
  ...
  def test_it_can_create_router
    router = Router.new("something", 0)
    assert router
  end

  def test_it_can_return_hello_string
    expected = "<pre>Hello, World! (1)</pre>"
    assert_equal expected, Router.new("/hello", 1).response
  end

  def test_it_can_return_time_string
    router = Router.new("/datetime", 1)
  ...
  def test_it_can_return_shutdown_string
    ...
    assert_equal expected, Router.new("/shutdown", 1).response
  end
